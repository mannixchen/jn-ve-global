import { RouteConfig } from '@jsjn/types/Route'

/**
 * 业务路由为 / 路由的后代路由，path 不需要加 '/'
 * 否则将被定义为根路径
 * 参考：https://next.router.vuejs.org/zh/guide/essentials/nested-routes.html
 *
 * 路由配置参数参考：https://next.router.vuejs.org/zh/api/#routerecordraw
 * 路由配置呈嵌套模式，只有拥有 component 的配置才会被真正注册呈路由，没有 component 仅做层级结构
 */
const routes: Array<RouteConfig> = [
    {
        path: 'table',
        name: 'table',
        meta: {
            title: '表格',
            icon: 'el-Grid'
        },
        children: [
            {
                path: 'index',
                name: 'index',
                meta: {
                    title: 'Table',
                    isBlurry: true
                },
                component: () => import('@/views/demo/table/index.vue')
            },
            {
                path: 'selection',
                name: 'selection',
                meta: {
                    title: 'Selection',
                    isBlurry: true
                },
                component: () => import('@/views/demo/table/selectionDemo.vue')
            },
            {
                path: 'hideDemo',
                name: 'hideDemo',
                meta: {
                    title: 'HideDemo',
                    isBlurry: true
                },
                component: () => import('@/views/demo/table/hideDemo.vue')
            },
            {
                path: 'flowHeight',
                name: 'flowHeight',
                meta: {
                    title: 'FlowHeight',
                    isBlurry: true
                },
                component: () => import('@/views/demo/table/flowHeight.vue')
            },
            {
                path: 'editDemo',
                name: 'editDemo',
                meta: {
                    title: 'EditDemo',
                    isBlurry: true
                },
                component: () => import('@/views/demo/table/editDemo.vue')
            }
        ]
    }
]

export default routes
